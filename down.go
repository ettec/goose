...
func Down(db db, dir string, opts ...OptionsFunc) error {
	option := &options{}
	for _, f := range opts {
		f(option)
	}
...
func DownTo(db db, dir string, version int64, opts ...OptionsFunc) error {
	option := &options{}
	for _, f := range opts {
		f(option)
	}
...
// target version.
func downToNoVersioning(db db, migrations Migrations, version int64) error {
